...
const int init_num_keys = 80000000;
const int batch_size = 1000000;
const int total_num_keys = 200000000;
// 50% lookup, 50% insert
const float insert_frac = 0.5;

int main(int argc, char* argv[])
{
    if (argc < 2) {
        std::cout << "Usage: ./alex [workload_file_path]" << std::endl;
    }
    // read keys from file
    ...
    is.read(reinterpret_cast<char*>(keys), std::streamsize(total_num_keys * sizeof(double)));
    is.close();
    // generate random values
    auto values = new std::pair<double, double>[init_num_keys];
    std::mt19937_64 gen_payload(std::random_device{}());
    for (int i = 0; i < init_num_keys; i++) {
        values[i].first = keys[i];
        values[i].second = static_cast<double>(gen_payload());
    }
    // create index and bulk load
    alex::Alex index;
    ...
    while (true) {
        ...
        for (int j = 0; j < num_lookups_per_batch; j++) {
            ...
            double* payload = index.get_payload(key);
            if (payload) {
                sum += *payload;
            }
        }
        ...
        for (; i < num_keys_after_batch; i++) {
            index.insert(keys[i], static_cast<double>(gen_payload()));
        }
